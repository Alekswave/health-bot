...
# Render usually provides this automatically for Web Service
...
def classify_bp(sys_: int, dia: int) -> str:
    if sys_ >= 180 or dia >= 120:
        return "🚨 Дуже високий тиск. Якщо погане самопочуття — звернись по невідкладну допомогу."
    if sys_ >= 140 or dia >= 90:
        return "⚠️ Підвищений тиск."
    if 120 <= sys_ <= 129 and dia < 80:
        return "ℹ️ Тиск трохи підвищений."
...
def validate_values(sys_: int, dia: int, pulse: int) -> str | None:
    if not (60 <= sys_ <= 260):
        return "Систолічний тиск виглядає некоректно. Спробуй формат: 120/80 68"
    if not (40 <= dia <= 160):
        return "Діастолічний тиск виглядає некоректно. Спробуй формат: 120/80 68"
    if dia >= sys_:
        return "Діастолічний не може бути більшим або рівним систолічному."
    if not (35 <= pulse <= 220):
        return "Пульс виглядає некоректно."
    return None
...
async def cmd_chart(message: Message):
    ...
    if len(rows) < 2:
        await message.answer("Для графіка потрібно хоча б 2 вимірювання за останні 7 днів.")
        return
...
# =====================================
# AIOHTTP APP FOR RENDER + WEBHOOK
# =====================================
